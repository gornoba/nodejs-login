;(() => {
  ...
  const ORIGINAL_LOGIN = 'naver_id_login_anchor'
  ...
  naver.setDomain(`http://${APP_CONFIG.HOST}`)
  ...
  const originalButton = document.getElementById(ORIGINAL_LOGIN)
  if (!originalButton) {
    throw new Error(
      `Failed to find original Naver login button with ID: ${ORIGINAL_LOGIN}. Did you forget to initialize the SDK?`
    )
  }
